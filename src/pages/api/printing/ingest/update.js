...
export default async function handler(req, res) {
    ...
    if (req.method === 'PUT') {
        ...
        const notes = body?.notes || '[AUTOGENERATED EVENT]';
        ...
        if (action === PrintStates.PRINTING) {
            ...
            if (!printerPre.queue.includes(print._id.toString())) {
                res.status(400).json({
                    success: false,
                    type: 'notinqueue',
                    message: 'Print not found in queue'
                });
                return;
            }
        ...
        } else if (action === PrintStates.CANCELED) {
            // update the print state to completed and add event
            const printerPre = await mongoClient.db('printing').collection('printers').findOne({
                id: print.printer
            });
